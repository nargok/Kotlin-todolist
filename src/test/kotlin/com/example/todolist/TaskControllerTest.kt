...
class TaskControllerTest {
    ...
    fun index_保存されているタスクを全件表示すること() {
        ...
        // whenはkotlinの予約後なので`で囲う
        Mockito.`when`(taskRepository.findAll()).thenReturn(tasks)

        mockMvc.perform(MockMvcRequestBuilders.get("/tasks"))
                .andExpect(view().name("tasks/index")) // viewがtasks/indexであること
                .andExpect(model().attribute("tasks", tasks)) // tasksという名前でtasksが設定されていること
    ...
    fun create_ポストされた内容でタスクを新規作成すること() {
        ...
        Mockito.verify(taskRepository).create("piyo") // taskRepositoryに対してcreate("piyo")という呼び出しが1回あったかどうか
    }
}